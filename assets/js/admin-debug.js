    ...
    window.SLWNProductFiltersAdmin = {
        ...
        initAllWidgets: function() {
            ...
            $('div.widget').each(function() {
                var $widget = $(this);
                var $filterTypeSelect = $widget.find('select[id*="filter_type"]');
                var $displayTypeSelect = $widget.find('select[id*="display_type"]');
                
                // Also check for our widget base ID patterns
                var widgetId = $widget.attr('id') || '';
                var isSlwnWidget = widgetId.indexOf('slwn_product_filter') !== -1 || 
                                 $filterTypeSelect.length > 0 || 
                                 $displayTypeSelect.length > 0;
                
                // Only process if this widget has our selects or is our widget
                if (isSlwnWidget && $filterTypeSelect.length && $displayTypeSelect.length) {
                    ...
                    self.initSingleWidget($widget, $filterTypeSelect, $displayTypeSelect);
                }
            });
        },
         ...
         */
        initSingleWidget: function($widget, $filterTypeSelect, $displayTypeSelect) {
            var self = this;
            ...
            $filterTypeSelect.on('change.slwn-admin', function() {
                console.log('SLWN Admin: Filter type changed to:', $(this).val());
                toggleFilterTypeFields();
            });
            
            $displayTypeSelect.on('change.slwn-admin', function() {
                console.log('SLWN Admin: Display type changed to:', $(this).val());
