...
(function ($) {
    ...
    /**
     * Sprawdź czy parametr URL to filter WooCommerce
     */
    function isFilterParam(paramName) {
        return (
            paramName.indexOf("pa_") === 0 || // atrybuty
            paramName === "product_cat" || // kategorie
            paramName.indexOf("min_pa_") === 0 || // range min
            paramName.indexOf("max_pa_") === 0
        ); // range max
    }

    /**
     * Pobierz nazwę atrybutu z parametru
     */
    function getAttributeFromParam(paramName) {
        if (paramName.indexOf("pa_") === 0) {
            return paramName.replace("pa_", "");
        }
        if (paramName.indexOf("min_pa_") === 0) {
            return paramName.replace("min_pa_", "");
        }
        if (paramName.indexOf("max_pa_") === 0) {
            return paramName.replace("max_pa_", "");
        }
        return null;
    }
    ...
    $(document).ready(function () {
        ...
        function getAllProductFilters() {
            ...
            $(".slwn-product-filter-widget-form").each(function () {
                ...
                $form
                    .find(
                        'select[name^="pa_"], select[name="product_cat"], input[name^="pa_"], input[name^="min_pa_"], input[name^="max_pa_"], input[name="product_cat"]'
                    )
                    .each(function () {
                        const $input = $(this);
            ...
            $(
                'input[type="hidden"][name^="min_pa_"], input[type="hidden"][name^="max_pa_"]'
            ).each(function () {
                const $input = $(this);
                const name = $input.attr("name");
            ...
            if (!hasFilters) {
                try {
                    const savedFilters =
                        localStorage.getItem("product_filters");
                    if (savedFilters) {
        ...
        function initCategoryChangeHandlers() {
            ...
            $('select[name="product_cat"]').on("change", function () {
                ...
                if ($selectedOption.val() === "") {
                    url =
                        $selectedOption.data("url") ||
                        $select.data("shop-url") ||
                        "/sklep/";
                } else {
            ...
            $(".slwn-product-filter--category .slwn-product-filter__radio").on(
                "change",
                function () {
                    const $radio = $(this);
        ...
        function restoreProductFilters() {
            try {
                ...
                $(
                    '.slwn-product-filter-widget-form select[name^="pa_"], .slwn-product-filter-widget-form select[name="product_cat"], .slwn-product-filter-widget-form input[name^="pa_"]:checked, .slwn-product-filter-widget-form input[name="product_cat"]:checked'
                ).each(function () {
                    const $input = $(this);
                    const name = $input.attr("name");
                ...
                if (!hasFiltersInURL && savedFilters) {
                    ...
                    const isShopPage =
                        currentPath.endsWith(shopPath) ||
                        currentPath.endsWith(shopPath.replace("/", ""));
                ...
                if (Object.keys(filtersToApply).length > 0) {
                    ...
                    $(".slwn-product-filter-widget-form").each(function () {
                        ...
                        $form
                            .find(
                                'select[name^="pa_"], select[name="product_cat"], input[name^="pa_"], input[name="product_cat"], input[name^="min_pa_"], input[name^="max_pa_"]'
                            )
                            .each(function () {
                                const $input = $(this);
                                const name = $input.attr("name");
                                    ...
                                    // Dla radio i checkbox
                                        ...
                                        if ($input.is('[type="checkbox"]')) {
                                            ...
                                            const $form = $input.closest(
                                                ".slwn-product-filter-widget-form"
                                            );
                                            const baseName = filterKey; // używaj filterKey zamiast name
                                            const $hiddenField = $form.find(
                                                'input[name="' +
                                                    baseName +
                                                    '"].checkbox-values-holder'
                                            );
                                            if ($hiddenField.length) {
                                                $hiddenField.val(
                                                    filterValue || ""
                                                ); // Ustaw pustą wartość jeśli filtr pusty
                                            }
                                        } else if (
                                            $input.val() === filterValue
                                        ) {
                                ...
                                } else if (name && isFilterParam(name)) {
                    ...
                    for (const filterName in filtersToApply) {
                        if (
                            filterName.includes("_min") ||
                            filterName.includes("_max")
                        ) {
                            const baseFilter = filterName
                                .replace("_min", "")
                                .replace("_max", "");
                            const minValue =
                                filtersToApply[baseFilter + "_min"];
                            const maxValue =
                                filtersToApply[baseFilter + "_max"];
                    ...
                    $(".slwn-product-filter-widget-form").each(function () {
                        ...
                        $form
                            .find(
                                'select[name^="pa_"], select[name="product_cat"]'
                            )
                            .each(function () {
                                const $select = $(this);
                                $select.val("");
                        ...
                        $form
                            .find(
                                'input[name^="pa_"][type="radio"], input[name^="pa_"][type="checkbox"], input[name="product_cat"][type="radio"], input[name="product_cat"][type="checkbox"]'
                            )
                            .prop("checked", false);
        ...
        function initResetFiltersButton() {
            $(".slwn-filters-buttons__reset, .wc-filter-reset").on(
                "click",
                function (e) {
        ...
        function init() {
            ...
            setTimeout(function () {
                ...
                setTimeout(function () {
                    $(
                        '.slwn-product-filter-widget-form select[name^="filter_"]'
                    ).each(function () {
                        const $select = $(this);
        ...
        function initWidgetFormHandlers() {
            ...
            $(document).on(
                "change",
                '.slwn-product-filter-widget-form select[name^="filter_"], .slwn-product-filter-widget-form input[name^="filter_"]',
                function () {
                    ...
                    if ($input.is('[type="checkbox"]') && name.endsWith("[]")) {
                        ...
                        const $checkboxes = $input
                            .closest(".slwn-product-filter-widget-form")
            ...
            $(document).on(
                "change",
                '.slwn-product-filter-widget-form input[type="hidden"][name*="filter_"]',
                function () {
                    // Pomiń jeśli jesteśmy w trakcie przywracania filtrów
                    if (isRestoringFilters) {
                        return;
            ...
            $(document).on(
                "change",
                '.slwn-product-filter-widget-form select[name="product_cat"]',
                function () {
